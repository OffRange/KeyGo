...
use crate::passkey::keygo_passkey::{PasskeyCodecError, passkey_from_bytes};
...
pub async fn provide_passkey(
    ...
    let request_options: PublicKeyCredentialRequestOptions =
        serde_json::from_str(json_request).map_err(|_| ProviderError::InvalidJsonFormat)?;
    ...
    let credential = client
        .authenticate(domain, options, client_data_hash)
        .await
