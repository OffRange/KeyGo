...
suspend fun <T> SecretData<T>.decryptSecretData(
    label: String,
    ctx: CoroutineContext = Dispatchers.Default,
): T = with(scope) {
    decryptedDataType.decode(
        CryptographicData(data, iv).decrypt(label = label, context = ctx)
    )
}

context(scope: CryptographicScope)
suspend inline fun <reified T> T.encryptSecretData(
    label: String,
    ctx: CoroutineContext = Dispatchers.Default,
): SecretData<T> = with(scope) {
    ...
    val encryptedData = encoded.encrypt(label = label, context = ctx)
    SecretData(
