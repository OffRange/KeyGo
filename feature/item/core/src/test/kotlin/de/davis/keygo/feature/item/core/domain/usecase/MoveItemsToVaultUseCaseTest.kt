...
import de.davis.keygo.core.security.crypto.FixedSession
...
class MoveItemsToVaultUseCaseTest {
    ...
    private val session = FixedSession.random()
    ...
    fun `same source and destination is a no-op success`() = runTest {
        ...
        assertEquals(seeded.keyInformation.wrappedKey.toList(), after.keyInformation.wrappedKey.toList())
    }
    ...
    fun `persist failure stops the loop and surfaces ItemMoveFailed`() = runTest {
        ...
        passwordRepository.failCreateOrUpdateForId = failingId to cause
        ...
        // First item migrated; failing item and the one after it remain in src.
    ...
    private suspend fun decrypt(password: Password, vault: Vault): Pair<String, String?> =
        cryptographicScopeProvider.itemScope(
            ...
            val totp: String? = password.totpSecret?.decryptSecretData(label = Password.LABEL_TOTP_SECRET)
            pw to totp
        }
}
